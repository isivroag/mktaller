...
Public Class frmconsultaclase
    ...
    Public Sub consulta()
        ...
        conn.consulta(grdatos, "select * from clase where estado_clase=1 order by id_clase")
        formatogrid()
    End Sub
    Private Sub formatogrid()
        DoubleBuffered = True
        grdatos.Columns(0).HeaderText = "ID"
        grdatos.Columns(1).HeaderText = "CLASE"
    ...
    End Sub
    Private Sub frmconsultaclase_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        consulta()
    ...
    Private Sub grdatos_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles grdatos.CellDoubleClick
        If e.RowIndex >= 0 Then
            datos.id_clase = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(0).Value)
            datos.nom_clase = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(1).Value)
            frmclase.Show()
            frmclase.mostrar(datos)
            frmclase.flag = 2
        End If
    ...
    Private Sub mncEDITAR_Click(sender As Object, e As EventArgs) Handles mncEDITAR.Click
        ...
        datos.id_clase = Convert.ToString(grdatos.CurrentRow().Cells(0).Value)
        datos.nom_clase = Convert.ToString(grdatos.CurrentRow().Cells(1).Value)
    ...
    Private Sub mncELIMINAR_Click(sender As Object, e As EventArgs) Handles mncELIMINAR.Click
        If MsgBox("Desea Eliminar este Clase", vbYesNo + vbQuestion,) = vbYes Then
            Dim clase As New C_Clase
            clase.id_clase = Convert.ToDouble(grdatos.CurrentRow().Cells(0).Value)
            conn.eliminar(clase, clase.id_clase.ToString)
            consulta()
        End If
    End Sub
    Sub exportarexcel()
