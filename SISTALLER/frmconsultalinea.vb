...
Public Class frmconsultalinea
    ...
    Public Sub consulta()
        ...
        conn.consulta(grdatos, "select * from linea where estado_linea=1 order by id_linea")
        formatogrid()
    End Sub
    Private Sub formatogrid()
        DoubleBuffered = True
        grdatos.Columns(0).HeaderText = "ID"
        grdatos.Columns(1).HeaderText = "LINEA"
    ...
    Private Sub frmconsultalinea_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        consulta()
    ...
    Private Sub grdatos_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles grdatos.CellDoubleClick
        If e.RowIndex >= 0 Then
            datos.id_linea = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(0).Value)
            datos.nom_linea = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(1).Value)

            frmlinea.Show()
            frmlinea.mostrar(datos)
            frmlinea.flag = 2
        End If
    ...
    Private Sub mncEDITAR_Click(sender As Object, e As EventArgs) Handles mncEDITAR.Click
        ...
        datos.id_linea = Convert.ToString(grdatos.CurrentRow().Cells(0).Value)
        datos.nom_linea = Convert.ToString(grdatos.CurrentRow().Cells(1).Value)
    ...
    Private Sub mncELIMINAR_Click(sender As Object, e As EventArgs) Handles mncELIMINAR.Click
        If MsgBox("Desea Eliminar este Linea", vbYesNo + vbQuestion,) = vbYes Then
            Dim linea As New C_Linea
            linea.id_linea = Convert.ToDouble(grdatos.CurrentRow().Cells(0).Value)
            conn.eliminar(linea, linea.id_linea.ToString)
            consulta()
        End If
    End Sub
    Sub exportarexcel()
