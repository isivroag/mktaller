...
Public Class frmconsultasubmarca
    ...
    Public Sub consulta()
        ...
        conn.consulta(grdatos, "select submarca.id_submarca,submarca.nom_submarca,submarca.id_marca,marca.nom_marca from submarca join marca on submarca.id_marca=marca.id_marca where estado_submarca=1 order by id_marca,id_submarca")
        formatogrid()
    End Sub
    Private Sub formatogrid()
        DoubleBuffered = True
        grdatos.Columns(0).HeaderText = "ID SUBMARCA"
        grdatos.Columns(1).HeaderText = "SUBMARCA"
    ...
    End Sub
    Private Sub frmconsultasubmarca_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        consulta()
    ...
    Private Sub grdatos_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles grdatos.CellDoubleClick
        If e.RowIndex >= 0 Then
            ...
            datos.id_submarca = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(0).Value)
            datos.nom_submarca = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(1).Value)
            frmsubmarca.Show()
            frmsubmarca.cmarca.Text = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(2).Value) & " " & Convert.ToString(grdatos.Rows(e.RowIndex).Cells(3).Value)
            frmsubmarca.mostrar(datos)
            frmsubmarca.flag = 2
        End If
    ...
    Private Sub mncEDITAR_Click(sender As Object, e As EventArgs) Handles mncEDITAR.Click
        ...
        datos.id_submarca = Convert.ToString(grdatos.CurrentRow().Cells(0).Value)
        datos.nom_submarca = Convert.ToString(grdatos.CurrentRow().Cells(1).Value)




        frmsubmarca.Show()
        frmsubmarca.cmarca.Text = Convert.ToString(grdatos.CurrentRow().Cells(2).Value) & " " & Convert.ToString(grdatos.CurrentRow().Cells(3).Value)
        frmsubmarca.mostrar(datos)
        frmsubmarca.flag = 2
    End Sub
    ...
    Private Sub mncELIMINAR_Click(sender As Object, e As EventArgs) Handles mncELIMINAR.Click
        If MsgBox("Desea Eliminar este registro", vbYesNo + vbQuestion,) = vbYes Then
            Dim submarca As New C_Submarca
            submarca.id_marca = Convert.ToDouble(grdatos.CurrentRow().Cells(2).Value)
            submarca.id_submarca = Convert.ToDouble(grdatos.CurrentRow().Cells(0).Value)
            conn.eliminar(submarca, submarca.id_marca.ToString, submarca.id_submarca.ToString)
            consulta()
        End If
    End Sub
    Sub exportarexcel()
