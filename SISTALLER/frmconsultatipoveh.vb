...
Public Class frmconsultatipoveh
    ...
    Public Sub consulta()
        ...
        conn.consulta(grdatos, "select * from tipo_veh where estado_tipo_veh=1 order by id_tipo_veh")
        formatogrid()
    End Sub
    ...
    End Sub
    Private Sub frmconsultatipoveh_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        consulta()
    ...
    Private Sub grdatos_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles grdatos.CellDoubleClick
        If e.RowIndex >= 0 Then
            datos.id_tipo_veh = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(0).Value)
            datos.nombre_tipo_veh = Convert.ToString(grdatos.Rows(e.RowIndex).Cells(1).Value)
            frmtipoveh.Show()
            frmtipoveh.mostrar(datos)
            frmtipoveh.flag = 2
        End If
    ...
    Private Sub mncEDITAR_Click(sender As Object, e As EventArgs) Handles mncEDITAR.Click
        ...
        datos.id_tipo_veh = Convert.ToString(grdatos.CurrentRow().Cells(0).Value)
        datos.nombre_tipo_veh = Convert.ToString(grdatos.CurrentRow().Cells(1).Value)
    ...
    Private Sub mncELIMINAR_Click(sender As Object, e As EventArgs) Handles mncELIMINAR.Click
        If MsgBox("Desea Eliminar este Vehiculo", vbYesNo + vbQuestion,) = vbYes Then
            Dim tipoveh As New C_Tipo_Veh
            tipoveh.id_tipo_veh = Convert.ToDouble(grdatos.CurrentRow().Cells(0).Value)
            conn.eliminar(tipoveh, tipoveh.id_tipo_veh.ToString)
            consulta()
        End If
    End Sub
    Sub exportarexcel()
